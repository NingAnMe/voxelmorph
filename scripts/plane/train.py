...
parser.add_argument('--img-list-val', required=True, help='line-seperated list of val files')
...
all_loss_val = list()  # 保存2个部分的loss
all_total_loss_val = list()  # 保存2个部分loss的和
best_epoch = 0
best_loss = np.inf
...
for epoch in range(args.initial_epoch, args.epochs):
    ...
    # save image loss
    if epoch % 10 == 0:
        plot_loss_img(all_total_loss, all_total_loss_val, os.path.join(model_dir, 'image_loss.png'))
    ...
    for step in range(args.steps_per_epoch):
        ...
        inputs = [minmaxnormalize(i) for i in inputs]
    ...
    for num in range(val_files_len):
        inputs, y_true = next(generator_val)
        inputs = [minmaxnormalize(i) for i in inputs]
        inputs = [torch.from_numpy(d).to(device).float().permute(0, 3, 1, 2) for d in inputs]
        y_true = [torch.from_numpy(d).to(device).float().permute(0, 3, 1, 2) for d in y_true]
    ...
    epoch_loss_val = np.mean(epoch_loss_val, axis=0)
    epoch_total_loss_val = np.mean(epoch_total_loss_val)
    # print epoch info
    epoch_info = 'Epoch %d/%d' % (epoch + 1, args.epochs)
    time_info = '%.4f sec/step' % np.mean(epoch_step_time)
    losses_info = ', '.join(['%.4e' % f for f in epoch_loss])
    loss_info = 'loss: %.4e  (%s)' % (epoch_total_loss, losses_info)
    losses_info_val = ', '.join(['%.4e' % f for f in epoch_loss_val])
    loss_info_val = 'loss: %.4e  (%s)' % (epoch_total_loss_val, losses_info_val)
    print(' - '.join(('train', epoch_info, time_info, loss_info)), flush=True)
    print(' - '.join(('valid', epoch_info, time_info, loss_info_val)), flush=True)

    # append epoch loss
    all_loss.append(epoch_loss)
    all_total_loss.append(epoch_total_loss)
    all_loss_val.append(epoch_loss_val)
    all_total_loss_val.append(epoch_total_loss_val)
